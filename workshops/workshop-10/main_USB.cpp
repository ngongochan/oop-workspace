...
int main(){
    ...
    for(int i = 0; i < max_usb_ports; i++){
        // CreateUsbConnection is a static function that creates a new object of USBConnection
        // if a port is available
        c[i] = USBConnection::CreateUsbConnection();

        if (c[i] != nullptr){
            std::cout << "USB connection with ID = " << c[i]->get_id() << " was created\n";
        } else {
    ...
    if (a != nullptr){
            std::cout << "USB connection with ID = " << a->get_id() << " was created\n";
    } else {
            std::cout << "no more USB ports available\n";
    }
